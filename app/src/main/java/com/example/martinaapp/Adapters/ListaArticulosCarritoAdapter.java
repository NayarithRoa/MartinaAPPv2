...
public class ListaArticulosCarritoAdapter  extends RecyclerView.Adapter<ListaArticulosCarritoAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull ListaArticulosCarritoAdapter.ViewHolder holder, int position) {
        ...
        /*holder.txttotalProdUnit.setText("$"+Math.round((listaArticulosSeleccionados.get(position).getNumberInCart()*listaArticulosSeleccionados.get(position).getVlr_unitario()));
        holder.txtCantArt.setText(String.valueOf(listaArticulosSeleccionados.get(position).getNumberInCart()));


        int drawableReourceId=holder.itemView.getContext().getResources().getIdentifier(listaArticulosSeleccionados.get(position).getPic(),"drawable",holder.itemView.getContext().getPackageName());
